...
export function buildOrganizationJsonLd() {
  ...
  return {
    ...ORG,
    url: siteUrl,
    logo: `${siteUrl}/logos/logo-primary.svg`,
  };
}
...
export function buildArticleJsonLd(post, category) {
  ...
  return {
    ...
    author: {
      '@type': 'Person',
      name: post.author_name || 'The Lily Letters Co.',
    },
    publisher: {
      '@type': 'Organization',
      name: 'The Lily Letters Co.',
...
export async function generateSitemap(supabase) {
  ...
  const entries = urls
    .map((u) => {
      const lastmod = u.lastmod ? `<lastmod>${new Date(u.lastmod).toISOString().split('T')[0]}</lastmod>` : '';
...
export async function generateLlmsTxt(supabase) {
  ...
  const lines = [
    ...
    '',
    '## About',
    'The Lily Letters Co. offers editable Canva wedding templates: invitations, signs, menus, seating charts, and bridal shower stationery.',
    '',
    '## Primary pages',
    `- Home: ${siteUrl}/`,
  ...
  lines.push('', '## Contact', '- Email: thelilyletters.co@gmail.com', '- Instagram: https://www.instagram.com/thelilyletters.co');
  return lines.join('\n');
}
