...
import { getAssetTags } from '../utils/indexTemplate.js';
...
function layout({ title, description, canonical, ogImage, body, jsonLd = [], preloadImage }) {
  const { isDev } = getConfig();
  const assets = getAssetTags(isDev);
  const meta = buildMetaTags({ title, description, canonical, ogImage, type: 'article' });
...
export function renderBlogIndex({ posts, categories, siteUrl }) {
...
export function renderCategoryArchive({ category, posts, siteUrl }) {
